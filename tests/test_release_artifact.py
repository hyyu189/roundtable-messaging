...
@pytest.mark.parametrize(
    ...
    [
        "_rtruntime.py",
        "roundtable_messaging-0.1.5.data/scripts/roundtable",
        "roundtable_messaging-0.1.5.data/scripts/_rtruntime.py",
        (
            "roundtable_messaging-0.1.5.data/data/share/roundtable/"
...
def test_project_wheel_validator_requires_runtime_helper_copies(tmp_path, missing):
    wheel = tmp_path / "roundtable_messaging-0.1.5-py3-none-any.whl"
    data_prefix = "roundtable_messaging-0.1.5.data/"
    required = {
    ...
    with pytest.raises(build_release.ReleaseError, match="missing required paths"):
        build_release._validate_project_wheel(wheel, "0.1.5")
...
def test_release_archive_is_deterministic_allowlisted_and_runtime_free(
    ...
    assert root == "roundtable-messaging-0.1.5"
    assert {
        "BUILD-METADATA.json",
        "CREDITS.md",
        "LICENSE",
    ...
    assert any(
        name.startswith("wheels/roundtable_messaging-0.1.5-")
